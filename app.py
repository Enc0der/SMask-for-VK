...
def get_file(message):
    ...
    bot.reply_to(
        message,
        f"Привет! Ого, какое шикарное исполнение! "
...
def create_file_paths_list():
    ...
    for i in range(len(file_paths_list)):
        s3_client.download_file(
            "splitedinputfiles", file_paths_list[i], "origin_input.wav"
        )
...
def making_return_video():
    ...
    for i in range(len(file_paths_list)):
        ...
        the_closet_note = notes_list[
            i
        ]  # Assuming notes_list is defined somewhere in the code
